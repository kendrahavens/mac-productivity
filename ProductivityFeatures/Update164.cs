...
namespace ProductivityFeatures
{
    class Update164
    {
        ...
        // Type (Alt + Enter) and select `Configure or Suppress issues`
        ...
        // Press (Alt + Enter) and select the option to `Add null checks for all parameters`
        ...
        // you want to navigate the inheritance hierarchy
        // Place your cursor on class D, right click, select Go To Base
        ...
        string LocalFunctionStatic()
        {
            ...
            // Press (Alt + Enter) and select Make local function `static`
            int MakeStatic()
            {
                int x = 1;
                return x += 1;
            ...
            // Press (Alt + Enter) and select `Pass variable explicitly in local static function`
            static int AddLocal()
            {
                return /*x +=*/ 1;
